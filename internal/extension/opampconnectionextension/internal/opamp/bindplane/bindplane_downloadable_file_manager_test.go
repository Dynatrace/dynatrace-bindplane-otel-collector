...
func TestGetOutputFilePath(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			basepath: filepath.Join("/", "tmp", "dynatrace-bindplane-otel-collector-update"),
			url:      "http://example.com/some-file.zip",
			out:      filepath.Join("/", "tmp", "dynatrace-bindplane-otel-collector-update", "some-file.zip"),
		},
		{
			name:     "Input url is valid tar",
			basepath: filepath.Join("/", "tmp", "dynatrace-bindplane-otel-collector-update"),
			url:      "http://example.com/some-file.tar.gz",
			out:      filepath.Join("/", "tmp", "dynatrace-bindplane-otel-collector-update", "some-file.tar.gz"),
		},
		{
