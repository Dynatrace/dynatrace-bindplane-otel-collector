...
| exclude_file_log_path | A list of file glob patterns to exclude from reading | []string | `[/var/log/containers/dynatrace-bindplane-otel-collector-* /var/log/containers/dbdot-*-collector-*]` | false |  |
...
| offset_storage_dir | The directory that the offset storage file will be created | string | `${env:BINDPLANE_COLLECTOR_HOME}/storage` | false |  |
...
receivers:
  plugin:
    ...
    parameters:
      ...
      exclude_file_log_path: [/var/log/containers/dynatrace-bindplane-otel-collector-* /var/log/containers/dbdot-*-collector-*]
      body_json_parsing: true
      start_at: end
      log_driver: auto
      offset_storage_dir: ${env:BINDPLANE_COLLECTOR_HOME}/storage
      parse: true
```
